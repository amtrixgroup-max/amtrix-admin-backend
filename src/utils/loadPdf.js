...
function bolFacts(load) {
  return [
    ...
    ['Distance', distanceText(load), 'Declared Value', declaredValue(load)],
  ]
}

function fillBol(doc, load, { blind = false } = {}) {
  ...
  drawStopsTable(doc, load, { signatures: !blind })
  if (blind) {
...
function fillLoadConfirmation(doc, load) {
  ...
  drawFactTable(doc, load, [
    ...
    ['Weight', weightText(load), 'Commodity', load.commodity],
    ['Distance', distanceText(load), 'Declared Value', declaredValue(load)],
    ['Last Free Day', formatShortDay(load.lastFreeDay) || formatDay(load.lastFreeDay), '', ''],
  ])
...
function fillRateConfirmation(doc, load) {
  ...
  drawFactTable(doc, load, [
    ...
    ['Declared Value', declaredValue(load), '', ''],
  ])
...
function writeDocument(kind) {
  switch (kind) {
    ...
    case 'blind-bol':
      return (doc, load) => fillBol(doc, load, { blind: true })
    default:
      return (doc, load) => fillBol(doc, load, { blind: false })
  }
}
